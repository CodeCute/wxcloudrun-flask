...
CREATE TABLE IF NOT EXISTS `NewsComments` (
    ...
    createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (news_id) REFERENCES News(id) ON DELETE CASCADE,
    FOREIGN KEY (parent_id) REFERENCES NewsComments(id) ON DELETE SET NULL
);

-- 结伴旅行相关表
CREATE TABLE IF NOT EXISTS `CompanionTags` (
    id INT PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `CompanionTagRelations` (
    companion_id INT NOT NULL,
    tag_id INT NOT NULL,
    PRIMARY KEY (companion_id, tag_id),
    FOREIGN KEY (companion_id) REFERENCES Companions(id) ON DELETE CASCADE,
    FOREIGN KEY (tag_id) REFERENCES CompanionTags(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS `CompanionReservations` (
    id INT PRIMARY KEY AUTO_INCREMENT,
    companion_id INT NOT NULL,
    user_id VARCHAR(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `CompanionReviews` (
    id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (reservation_id) REFERENCES CompanionReservations(id) ON DELETE CASCADE,
    FOREIGN KEY (companion_id) REFERENCES Companions(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS `SolutionApplications` (
    id INT PRIMARY KEY AUTO_INCREMENT,
...
-- 向导标签测试数据
INSERT INTO CompanionTags (name) VALUES
...
-- 向导标签关系测试数据
INSERT INTO CompanionTagRelations (companion_id, tag_id) VALUES
(1, 1), (1, 3), (1, 7),
(2, 2), (2, 4), (2, 3),
(3, 1), (3, 4), (3, 6);
...
-- 关于我们信息测试数据
INSERT INTO AboutInfo (title, content, type) VALUES
